...
try:
    ...
    def research_company(company_url, company_name):
        ...
        # Targeted query to detect Migration vs SRE triggers
    ...
    def calculate_hammer_math(agent_count, scenario_type, playbook):
        ...
        if scenario_type == "operations" or scenario_type == "hybrid":
            ...
            # 1. Hammer Edge (Endpoint Assurance)
            # Logic: 57% of tickets are endpoint. Edge detects 100%. Deflects Tier 1 costs.
            ...
            # 2. Hammer Ativa (Network Assurance)
            # Logic: Reduce War Rooms via DPI & MTTI reduction
            ...
            # 3. Hammer VoiceWatch (TFN Monitoring)
            # Logic: Automating manual testing of TFNs
        ...
        if scenario_type == "migration" or scenario_type == "hybrid":
            ...
            # 4. Hammer Performance (Load Testing)
            # Logic: Avoid Failed Cutover (Rollback) + Brand Damage
            ...
            # 5. Hammer QA (Automated Testing)
            # Logic: Automated Regression vs Manual Testing
    ...
    def create_pdf(data, company_name, filename="Hammer_ROI_Report.pdf"):
        ...
        pdf.cell(90, 10, txt="Logic / Driver", border=1)
        pdf.ln()
        ...
        for product, details in data['breakdown'].items():
            if details['value'] > 0:
                ...
                pdf.cell(40, 10, txt=f"${details['value']:,.0f}", border=1, align='R')
                # Truncate narrative to fit
                narrative = (details['narrative'][:55] + '..') if len(details['narrative']) > 55 else details['narrative']
                pdf.cell(90, 10, txt=narrative, border=1)
                pdf.ln()
    ...
    # --- MAIN EXECUTION FLOW ---
    print("--- Starting Hammer ROI V1.2 (Full Portfolio) ---")
...
# --- ERROR HANDLING ---
    ...
    traceback.print_exc()
    # (Error PDF Code Omitted for brevity, assume same as V1.1)
